...
@dp.table(
name="riskpulse.gold.risk_card_alert"
,comment="Alert details where transaciton has been performed with value higher than what is configured by customer"
)
def risk_card_alert() -> DataFrame:
    transactions=spark.readStream.table("riskpulse.silver.transactions")
    risk_watchlist=spark.readStream.table("riskpulse.silver.risk_watchlist")

    customers=spark.read.table("riskpulse.silver.customers")

    transactions_with_watermark=transactions.withWatermark("transaction_timestamp", "5 minutes")
    risk_watchlist_with_watermark=risk_watchlist.withWatermark("effective_from", "5 minutes")

    risk_detected=(
            transactions_with_watermark.join(
                    risk_watchlist_with_watermark,
                    transactions_with_watermark.card_number==risk_watchlist_with_watermark.entity_id,
                    "inner"
            ).join(
                customers,
            ...
            risk_watchlist_with_watermark.city.alias("watchlist_city"),
            risk_watchlist_with_watermark.country.alias("watchlist_country")
